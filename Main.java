...
public class Main {
	...
	* 	default ones to run the simulation.
	*/
			...
			for (int i = 0; i < col; i++) {
				for (int j = 0; j < row; j++) {
					if (all_Location[i][j].max_grain_here != 0) {
						all_Location[i][j].grains_here = all_Location[i][j].max_grain_here;
	...
	// The same going Up and Down, any limit with conect to the opposite side
	// This funcition helps to evaluate the next location starting from a
	// defined one.

	// These methods are used when looking for the next location and when
